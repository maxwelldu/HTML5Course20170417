...
$.ajax({
    ...
    'success': function (response) {
        ...
        for(var i=0;i<response.data.length;i++){
            ...
            $('#nav-ul').append('<li><a href="list2.html?cat_id='+ obj.cat_id +'">'+ obj.cat_name +'</a></li>')
        }
...
//
var osearch1 = $('#osearch');
if(osearch1.length === 1){
    osearch1.click(function () {
        location.href="search.html?search_text=" + $('#search_text').val();
    });
}

if(localStorage.getItem('token')){
    $("#top-2").html(localStorage.getItem('username'));
}


$('#tuichu').click(function () {
      localStorage.clear();
       $('#top-2').html('<a id="denglu" href="login.html">登陆</a><a id="zhuce" href="biaodanyanzheng.html">注册</a>');
